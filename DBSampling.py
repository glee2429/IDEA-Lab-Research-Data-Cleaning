...
mu = int(statistics.mean(DB_Index)) # Mean
sigma = int(statistics.pstdev(DB_Index)) # Standard Deviation

k = int(input ("Enter the number of DB you'd like to sample: "))
sample = np.random.normal(mu, sigma, k).round(0)
#sample_int = np.round(sample)

print("My indexes of sampled DB : ", sample)
